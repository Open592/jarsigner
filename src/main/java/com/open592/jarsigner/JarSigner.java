...
public final class JarSigner {
    ...
    // JarSigner options:
    private final ArrayList<String> digestalgs = new ArrayList<>();
    ...
    /**
     * Returns the digest algorithms for this {@code JarSigner}.
     * <p>
     * The return value is never null.
     *
     * @return the digest algorithms.
     */
    public ArrayList<String> getDigestAlgorithms() {
        return digestalgs;
    }
    ...
    private void sign0(ZipFile zipFile, OutputStream os)
        ...
        try {
            digests = new MessageDigest[digestalgs.size()];
            for (int i = 0; i < digestalgs.size(); i++) {
                if (digestProvider == null) {
                    digests[i] = MessageDigest.getInstance(digestalgs.get(i));
                } else {
                    digests[i] = MessageDigest.getInstance(
                            digestalgs.get(i), digestProvider);
                }
            }
    ...
    private Attributes getDigestAttributes(
        ...
        // Record digest algorithms for each entry
        List<String> digestAlgorithms = Arrays.stream(digests).map(MessageDigest::getAlgorithm).toList();
        String digestAlgorithmsAttribute = String.join(" ", digestAlgorithms);
        attrs.putValue("Digest-Algorithms", digestAlgorithmsAttribute);
